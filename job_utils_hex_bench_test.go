...
func encodeBytesToFixedHex_32_Uint16LUTLower(dst []byte, src []byte) {
	_ = dst[63]
	_ = src[31]
	j := 0
	for i := 0; i < 32; i += 2 {
		w := hexUint16LowerQuadLUT[uint16(src[i])<<8|uint16(src[i+1])]
		dst[j+0] = byte(w >> 24)
		dst[j+1] = byte(w >> 16)
		dst[j+2] = byte(w >> 8)
		dst[j+3] = byte(w)
...
func encodeBytesToFixedHex_32_ByteLUTLower(dst []byte, src []byte) {
	_ = dst[63]
	_ = src[31]
	j := 0
	for i := 0; i < 32; i++ {
		p := hexByteLowerPairLUT[src[i]]
		dst[j+0] = byte(p >> 8)
		dst[j+1] = byte(p)
		j += 2
	}
...
func BenchmarkEncodeBytesToFixedHex_32_Uint16LUT_Lower(b *testing.B) {
	var src [32]byte
	for i := range src {
		src[i] = byte(i)
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		encodeBytesToFixedHex_32_Uint16LUTLower(out[:], src[:])
	}
}

func BenchmarkEncodeBytesToFixedHex_32_ByteLUT_Lower(b *testing.B) {
	var src [32]byte
	for i := range src {
		src[i] = byte(i)
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		encodeBytesToFixedHex_32_ByteLUTLower(out[:], src[:])
	}
}
